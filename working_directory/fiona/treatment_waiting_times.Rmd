...
#  filter(hbt == "S08000025") %>% 
  ...
  mutate(total_waiting_by_date = sum(number_waiting, na.rm = TRUE),
         total_waiting_over12weeks_by_date = sum(number_waiting_over12weeks, 
                                                    na.rm = TRUE)) %>% 
  ggplot(aes(month_ending, total_waiting_by_date)) + 
  geom_line() +
  geom_line(aes(month_ending, total_waiting_over12weeks_by_date))
```
